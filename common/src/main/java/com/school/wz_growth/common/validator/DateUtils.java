...
public class DateUtils {
	...
	private static final ThreadLocal<DateFormat> tl = new ThreadLocal<DateFormat>() {
		...
		protected DateFormat initialValue() {
			return new SimpleDateFormat(COMMON_DATE_FORMAT);
	 ...
	 * @return yyyyMMdd格式的年月日字符串
	 */
	public static String convertDateToYearMonthDay2(Date d) {
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd",
				new DateFormatSymbols());
		return df.format(d);
	}
...
//	}
	...
	public static String getBeforeYesterday() {
		Date cDate = new Date();
		cDate.setTime(cDate.getTime() - 24*2 * 3600 * 1000);
		SimpleDateFormat cSimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
		return cSimpleDateFormat.format(cDate);
	}
...
//	}
	...
	public static String friendlyDate(Date date, boolean minimalFormat) {
		return DateUtils
				.format(date, DateUtils.friendlyDateFormat(minimalFormat));
	}
	 ...
	 * @return
	 */
	public static List<Date> monthsBetween(Date from, Date to) {
		List<Date> result = new ArrayList<Date>();
		Calendar cal = Calendar.getInstance();
		cal.setTime(from);
	...
	public static String formatNowDate(){
	    return format(new Date(), COMMON_DATE_FORMAT);
	}
	 ...
	 * @return
	 */
	public static boolean subtractionEndMinute(String startTime, int timeNum) throws Exception {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date start = sdf.parse(startTime);
		Date end = sdf.parse(sdf.format(new Date()));
		long cha = end.getTime() - start.getTime();
	 ...
	 * 比较2个时间大小
	 * */
	public static int compareDateStr(String s1, String s2){
	...
	/**
	 * 比较两个年月型日期的大小，日期格式为HHmm 两个字串4位，前2代表时，后2代表分， <br>
	 * IF 第一个代表的时间 > 第二个代表的时间，返回真，ELSE 返回假 <br>
	 *
	 * @param s1
	 *            日期1
	 ...
	 * @return boolean 如果s1大于等于s2则返回真，否则返回假
	 */
	public static boolean hhMMGreatEqual(String s1, String s2) {
		String temp1 = s1.substring(0, 2);
		String temp2 = s2.substring(0, 2);
		String temp3 = s1.substring(2, 4);
		String temp4 = s2.substring(2, 4);
	...
	public  static String getAge(Date birthDay){
		 ...
		 *  返回秒级时间戮
		 * */
	    public static long dateStringToTimeNun(String dateStr, String fm) throws Throwable{

			long time = (new SimpleDateFormat(fm)).parse(dateStr).getTime()/1000;
			return time;
		}
     ...
     *  返回date
     * */
    public static String TimeNunToYMDDate(long num){

        SimpleDateFormat format =  new SimpleDateFormat("yyyy-MM-dd");
        Long time= new Long(num*1000);
        return format.format(time);
    }

	/**
	 *  yyyy-MM-dd的日期 start_day_str - end_day_str
	 *  返回date
	 * */
	public static long compareYMDDateStrBetweenDayNum(String start_day_str, String end_day_str){

		SimpleDateFormat formatter =  new SimpleDateFormat("yyyy-MM-dd");
		Date date1 = null, date2 = null;
		try {
			date1 = formatter.parse(start_day_str);
			date2 = formatter.parse(end_day_str);
	...
	/**
	 *  date_str（yyyy-MM-dd） + num(天数)
	 * @param date_str
	 * @param num  正数把日期往后增加一天,整数  往后推,负数往前移动
	 * @return
	 */
	public static String addDayNumByDayStr(String date_str,int num){
		...
		try {
		    ...
		    Date res_date=calendar.getTime(); //这个时间就是日期往后推一天的结果

			return formatter.format(res_date);
		} catch (ParseException e) {
			e.printStackTrace();
		}
	 ...
	 * @throws Exception
	 */
	public static String addAndSubtractDate(Date date, String type, int into) throws Exception {
		String Sdate = "";
		try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(date);
			...
			if (type.equals("D"))
			{
                ...
                calendar.add(Calendar.DAY_OF_MONTH, into);
			} else if (type.equals("M"))
			{
                formatter = new SimpleDateFormat("yyyy-MM");
                calendar.add(Calendar.MONTH, into);

			} else if (type.equals("Y")) {
                formatter = new SimpleDateFormat("yyyy");
                calendar.add(Calendar.YEAR, into);
			}else {
				throw new Exception("类型不对");
			}

            Date res_date=calendar.getTime();
			Sdate = formatter.format(res_date);
			return Sdate;
		} catch (Exception e) {
			throw e;
		}
...
//            params.put("start_date",requestVo.getC_s_date());
//
         try {

             String end_date = DateUtils.addAndSubtractDate(DateUtils.convertYMDStringToDate("2021-05-31"), "D",-30);
...
////
             ...
             Calendar calendar = Calendar.getInstance();
             calendar.setTime(dateFormat.parse(t));
